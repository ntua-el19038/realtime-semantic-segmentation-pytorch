...
GT_FORMAT_NAME = '{pid}_label.nrrd'
...
class ACDC(Dataset):
    def __init__(self, config,  mode='val', use_cache=False,
                 ...
                 gt_format_name = GT_FORMAT_NAME, ignore_black_slice=True, debug=False):
        # self.data_root = config.data_root
        self.val_folder = './datasets/vol/biomedic3/cc215/data/ACDC/ACDC_artefacted/RandomBias'
        self.split = mode
        self.root_dir='./datasets/vol/biomedic3/cc215/data/ACDC/preprocessed/'
        self.root_name_dir='/datasets/vol/biomedic3/cc215/data/ACDC/preprocessed/'
        self.data_setting_name=data_setting_name
        self.cval=cval
        self.image_format_name = image_format_name
        self.label_format_name = label_format_name
        self.gt_format_name = gt_format_name
        ...
        if self.split == 'train':
            ...
            print('load {},  containing {}, found {} slices'.format(
                self.val_folder, len(self.patient_id_list), self.datasize))
        ...
        if use_cache and os.path.exists(cache_file_path):
            ...
            for pid in patient_id_list:
                if(self.split == 'train'):
                    ...
                    img_path = os.path.join(self.val_folder, self.image_format_name.format(pid=pid))
                    label_path = os.path.join(self.val_folder, self.gt_format_name.format(pid=pid))
                try:
    ...
    def get_pid_list(self, identifier, cval):
        ...
        if self.split == 'train':
            all_dirs_list = sorted(os.listdir(self.root_dir))
            all_p_id_list1 = sorted(os.listdir(self.root_dir+all_dirs_list[0]))
            all_p_id_list2 = sorted(os.listdir(self.root_dir+all_dirs_list[1]))
            train_val_ids = []
            for pid in all_p_id_list1:
                x=pid.split('_')
                x=x[0]
                newpid1 = all_dirs_list[0]+'/'+x
                if newpid1 in train_val_ids:
                    continue
                else:
                    train_val_ids.append(newpid1)
                newpid2=all_dirs_list[1]+'/'+x
                if newpid2 in train_val_ids:
                    continue
                else:
                    train_val_ids.append(newpid2)
            all_p_id_list = train_val_ids
        else:
            all_dirs_list = sorted(os.listdir(self.val_folder))
            train_val_ids = []
    ...
    def load_patientImage_from_nrrd(self, patient_id, new_spacing=None, normalize=False):
        if "pid" in self.image_format_name:
            ...
            if(self.split=="train"):
                ...
                label_name = self.gt_format_name.format(pid=patient_id)
                img_path = os.path.join(self.val_folder, img_name)
                label_path = os.path.join(self.val_folder, label_name)
...
if __name__ == '__main__':
    ...
    print(image.shape)
    print(mask.shape)
    image_nd, label_nd, image, label_image = pit.load_img_label_from_path("./datasets/vol/biomedic3/cc215/data/ACDC/preprocessed/ES/002_img.nrrd", label_path = "./datasets/vol/biomedic3/cc215/data/ACDC/preprocessed/ES/002_seg.nrrd")
