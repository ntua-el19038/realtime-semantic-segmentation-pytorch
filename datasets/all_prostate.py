...
class AllProstateDataset(Dataset):
    def __init__(self, config,  mode='val', use_cache=False,
        ...
        self.val_folder = config.test_data_folder
        self.split = mode
        ...
        if use_cache and os.path.exists(cache_file_path):
            ...
            for pid in patient_id_list:
                if(self.split == 'train'):
                    ...
                    label_path = os.path.join(self.val_folder, self.label_format_name.format(pid=pid))
                try:
    ...
    def get_pid_list(self, identifier, cval):
        ...
        # all_datasets_list=sorted(os.listdir(self.root_dir))
        # all_p_id_list=[]
        all_p_id_list = sorted(os.listdir(self.root_dir))
        val_p_id_list = sorted(os.listdir(self.val_folder))
        # for dataset in all_datasets_list:
        #     dataset_pid_list = sorted(os.listdir(self.root_dir + dataset))
        #     for pid in dataset_pid_list:
        #         all_p_id_list.append(dataset + '/' + pid)
        test_ids = []
...
if __name__ == '__main__':
    ...
    pit = AllProstateDataset(config="l", mode='train')
    ...
    image_nd, label_nd, image, label_image = pit.load_img_label_from_path("./datasets/prostate_dataset/reorganized/A-ISBI/patient_1/t2_img_clipped.nii.gz", label_path = "./datasets/prostate_dataset/reorganized/A-ISBI/patient_1/label_clipped.nii.gz")
