...
class Synthia_Gta(Dataset):
    ...
    def __init__(self, config, mode='train'):
        ...
        if mode == 'train':
            self.transform = AT.Compose([
                ...
                AT.ColorJitter(brightness=config.brightness, contrast=config.contrast,
            ...
            self.transform = AT.Compose([
                AT.Resize(height=config.crop_size, width=config.crop_size),  # remove for cityscapes evaluation
                AT.PadIfNeeded(min_height=config.crop_h, min_width=config.crop_w, value=(114, 114, 114),
    ...
    def __getitem__(self, index):
        ...
        # print(f"Transformed shapes - Image: {augmented['image'].shape}, Mask: {augmented['mask'].shape}")
