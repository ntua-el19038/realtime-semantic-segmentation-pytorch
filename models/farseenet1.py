...
class FarSeeNet1(nn.Module):
    def __init__(self, num_class=1, n_channel=1, backbone_type='mixtransformer', act_type='relu'):
        super(FarSeeNet1, self).__init__()
        if 'mixtransformer' in backbone_type:
            self.frontend_network = MixVisionTransformer1.from_type(5)
            ...
            # elif 'mixtransformer' in backbone_type:
            #     self.frontend_network = MixVisionTransformer.from_type(0)
            # for param in self.frontend_network.parameters():
